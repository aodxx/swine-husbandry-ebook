...
const defaultPrefs: ReaderPrefs = { theme: 'light', fontScale: 1, lineHeight: 1.85, readingMode: 'reading', soundEnabled: true }
...
const availableParts = ((tocData as any).parts ?? [])
  .map((part: any) => ({
    ...
    chapters: (part.chapters ?? [])
      .map((chapter: any) => ({ ...chapter, topics: (chapter.topics ?? []).filter((item: any) => availableTopicIds.has(String(item.id))) }))
...
function App() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    loadSearchCorpus().then(entries => { if (!cancelled) { setSearchEntries(entries); setSearchIndexReady(true) } }).catch(() => undefined)
  ...
  useEffect(() => {
    ...
    loadTopic(topicId).then(loaded => { if (!cancelled) setTopic(loaded) }).catch(() => { if (!cancelled) setTopicError('ไม่สามารถเปิดเนื้อหาหัวข้อนี้ได้') }).finally(() => { if (!cancelled) setTopicLoading(false) })
    return () => { cancelled = true }
  }, [view, topicId])
  useEffect(() => {
  ...
  function openTopic(id: string, scrollY = 0) { setTopicId(id); setTopic(null); setResume({ topicId: id, scrollY }); saveProgress({ topicId: id, scrollY, updatedAt: Date.now() }); setView('read') }
  ...
  function handleContentClick(event: React.MouseEvent<HTMLElement>) {
    ...
    if (citation) { const citationIndex = Number(citation.dataset.citationIndex); const sourceId = topic.sourceIds[citationIndex - 1]; const source = sourceId ? sourceMap.get(sourceId) : undefined; if (source) setOverlay({ kind: 'source', source, citationIndex }); return }
    const glossary = event.target.closest<HTMLElement>('[data-glossary-id]')
    if (glossary?.dataset.glossaryId) { const term = glossaryMap.get(glossary.dataset.glossaryId); if (term) setOverlay({ kind: 'glossary', term }); return }
  ...
  return <main className={`reader mode-${prefs.readingMode}`} style={readerStyle}>
    ...
    {prefs.readingMode === 'book' ? <BookReader html={topic.html} title={topic.title} chapter={topic.chapter} fontScale={prefs.fontScale} lineHeight={prefs.lineHeight} soundEnabled={prefs.soundEnabled} hasPreviousTopic={Boolean(previous)} hasNextTopic={Boolean(next)} onPreviousTopic={() => previous && openTopic(previous.id)} onNextTopic={() => next && openTopic(next.id)} onOpenToc={() => setView('toc')} onContentClick={handleContentClick} /> : <><article className="prose" onClick={handleContentClick} dangerouslySetInnerHTML={{ __html: topic.html }} /><nav className="bottomnav"><button disabled={!previous} onClick={() => previous && openTopic(previous.id)}>← ก่อนหน้า</button><button onClick={() => setView('toc')}>สารบัญ</button><button disabled={!next} onClick={() => next && openTopic(next.id)}>ถัดไป →</button></nav></>}
    {settingsOpen && <div className="modal-backdrop" role="presentation" onClick={() => setSettingsOpen(false)}><section className="settings-sheet" role="dialog" aria-modal="true" aria-label="การตั้งค่าการอ่าน" onClick={event => event.stopPropagation()}><header><div><small>Reader Settings</small><h2>การตั้งค่าการอ่าน</h2></div><button onClick={() => setSettingsOpen(false)} aria-label="ปิด">×</button></header><fieldset><legend>โหมดอ่าน</legend><div className="theme-grid mode-grid"><button className={prefs.readingMode === 'reading' ? 'selected' : ''} onClick={() => updatePrefs({ readingMode: 'reading' })}>Reading Mode</button><button className={prefs.readingMode === 'book' ? 'selected' : ''} onClick={() => updatePrefs({ readingMode: 'book' })}>Flipbook Mode</button></div></fieldset><fieldset><legend>เสียงพลิกหน้ากระดาษ</legend><div className="theme-grid mode-grid"><button className={prefs.soundEnabled ? 'selected' : ''} onClick={() => updatePrefs({ soundEnabled: true })}>เปิดเสียง</button><button className={!prefs.soundEnabled ? 'selected' : ''} onClick={() => updatePrefs({ soundEnabled: false })}>ปิดเสียง</button></div></fieldset><label>ขนาดตัวอักษร <output>{Math.round(prefs.fontScale * 100)}%</output><input type="range" min="0.9" max="1.3" step="0.05" value={prefs.fontScale} onChange={event => updatePrefs({ fontScale: Number(event.target.value) })}/></label><label>ระยะห่างบรรทัด <output>{prefs.lineHeight.toFixed(2)}</output><input type="range" min="1.55" max="2.15" step="0.1" value={prefs.lineHeight} onChange={event => updatePrefs({ lineHeight: Number(event.target.value) })}/></label><fieldset><legend>ธีม</legend><div className="theme-grid">{(['light','sepia','dark'] as Theme[]).map(theme => <button key={theme} className={prefs.theme === theme ? 'selected' : ''} onClick={() => updatePrefs({ theme })}>{theme === 'light' ? 'กระดาษอุ่น' : theme === 'sepia' ? 'ซีเปีย' : 'กลางคืน'}</button>)}</div></fieldset><p className="settings-note">Flipbook Mode พลิกกระดาษแบบ 3D และลากจากขอบหน้าได้ หากระบบตั้งค่า Reduced Motion จะตัด animation ออกโดยยังเปลี่ยนหน้าได้ตามปกติ</p></section></div>}
